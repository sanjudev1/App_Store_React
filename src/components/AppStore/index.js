...
class AppStore extends Component {
  ...
  render() {
    ...
    const filterAppList = appsList.filter(each => each.category === activeTab)

    const filterAppListSearch = filterAppList.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    console.log(filterAppListSearch)
      ...
      <>
        <div className="bg-container">
          ...
          <ul className="appContainer">
            {filterAppListSearch.map(each => (
              <AppItem categoryList={each} key={each.appId} />
            ))}
          </ul>
        </div>
      </>
